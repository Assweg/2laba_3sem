...
public:
    ...
    void Tremove(const T& value); // Добавляем метод для удаления вершины
};
...
template <typename T>
void CompleteBinaryTree<T>::Tremove(const T& value) {
    int index = -1;
    for (int i = 0; i < size; i++) {
        if (tree[i] == value) {
            index = i;
    ...
    if (index == -1) {
        cout << "Вершина с заданным значением не найдена.\n";
        return;
    }
    ...
    // Если вершина имеет одного ребенка
    if (2 * index + 1 < size && 2 * index + 2 >= size) {
        tree[index] = tree[2 * index + 1];
        for (int i = 2 * index + 1; i < size - 1; i++) {
            tree[i] = tree[i + 1];
        }
        size--;
        return;
    ...
    if (2 * index + 1 >= size && 2 * index + 2 < size) {
        ...
        for (int i = 2 * index + 2; i < size - 1; i++) {
            tree[i] = tree[i + 1];
        }
        size--;
        return;
    ...
    // Если вершина имеет двух детей
    int minIndex = 2 * index + 2;
    while (2 * minIndex + 1 < size) {
